...
export default function Map() {
  ...
  useEffect(() => {
    if (window.kakao && mapRef.current) {
      const options = {
        center: new window.kakao.maps.LatLng(37.47885, 127.0114),
        level: 2, //확대됐을 때
      };
      const map = new window.kakao.maps.Map(mapRef.current, options); //지도생성
      const markerPosition = new window.kakao.maps.LatLng(37.47885, 127.0114); //마커가 표시될 위치
      //마커 생성
      ...
      <section className={cssStyle.subMap}>
        ...
        <Tabs defaultActiveKey='home' id='fill-tab-example' className='mb-3' fill>
          <Tab eventKey='home' title='대중교통 이용안내'>
            ...
            <p>
              <span>도로명주소</span>(우06757) 서울시 서초구 남부순환로 2406 예술의전당
              <br />
            </p>
